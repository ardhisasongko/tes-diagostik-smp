...
- `api/` — Vercel serverless functions yang dipakai deploy produksi:
  ...
  - `api/_lib/store.ts` — penyimpanan di **Supabase** (bukan Vercel Blob). Tabel `students(id, created_at, data jsonb)` — satu baris per siswa agar submit konkuren aman, plus tabel `settings(key, data)`. Nonaktif bila `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` tidak ada. Schema ada di `supabase-schema.sql`.
...
- Mode kunci siswa disimpan global via Supabase `settings`; PIN dibuka kembali oleh guru.
- Deploy produksi memakai endpoint di `api/`, bukan `server.ts`.
- Migrasi data `localStorage` ke server berjalan otomatis saat app dibuka (backup ke Supabase lalu reset cache lokal).
- `GURU_UNLOCK_MINUTES` (default 45) mengontrol berapa lama unlock mode guru berlaku sebelum kembali ke mode siswa.

## Riwayat Keputusan (dari git log)

- Vercel Blob (bukan KV) untuk penyimpanan siswa; satu file per siswa agar submit konkuren aman.
- Ganti ke Supabase karena kuota Blob Hobby cepat habis (usage threshold). Tabel `students` satu baris per siswa tetap mempertahankan submit konkuren aman.
...
- Mode siswa jadi default; unlock guru aktif dalam `GURU_UNLOCK_MINUTES` lalu otomatis kembali.
